	...
	if (!int_stage->busy && !int_stage->stalled) {
		...
		if (strlen(insToExec.opcode) > 0) {
			...
			if (strcmp(int_stage->opcode, "BZ") == 0) {
				int flag;
				int tempSID = cpu->rob->get_slot_id_from_cfid(int_stage->CFID,
						int_stage->pc);
				...
				if (flag == 1) {
					...
					//FLUSH ROB
					cpu->rob->flush_ROB_entries(tempSID);
					...
					itr = find(cfidDeque.begin(), cfidDeque.end(), int_stage->CFID);
					for (; itr != cfidDeque.end(); itr++) {
						int tempCFID = *itr;
						cpu->iq->flushIQEntries(tempCFID);
			...
			if (strcmp(int_stage->opcode, "ADDL") == 0) {
				...
				cpu->rob->update_ROB_slot(int_stage->pc, int_stage->CFID,
			...
			if (strcmp(int_stage->opcode, "ADD") == 0) {
				int buffer = int_stage->rs1_value + int_stage->rs2_value;
				cpu->urf->URF_Table[int_stage->u_rd] = buffer;
				cpu->urf->URF_TABLE_valid[int_stage->u_rd] = 1;
...
// Mul instruction spends 2-cycles.
	if (!stage->busy && !stage->stalled) {
		...
		if (iMulCycleSpent == 2) {
			...
			cpu->urf->URF_Z[stage->u_rd] = stage->zeroFlag;

			cpu->rob->update_ROB_slot(stage->pc, stage->CFID, stage->zeroFlag,
					VALID, buffer);
